...
class G_synthesis(nn.Module):
    ...
    def forward(self, styles, return_indermediate=False):
        intermediate = []
        for i, (res, m) in enumerate(self.blocks.items()):
            if i > 0:
                intermediate.append((res, x))
...
class StyleGAN(nn.Module):
    ...
    def ys_to_rgb(self, ys, return_intermediate=False):
        return self.g_synthesis(ys, return_intermediate)
    ...
    @staticmethod
    def export_from_tf(pickle_path):
        sys.path.append("models/stylegan1")
        ...
        weights_pt = [
            collections.OrderedDict([(k, torch.from_numpy(v.value().eval())) for k, v in w.trainables.items()])
            for w in weights]
        ...
        def key_translate(k):
            ...
            if "epi1.style_mod" in k or "epi2.style_mod" in k:
                k = k.replace("g_synthesis.blocks", "g_style.style_layers").replace(".epi1.style_mod", "_0").replace(
                    ".epi2.style_mod", "_1")
            return k
        ...
        param_dict = {key_translate(k): weight_translate(k, v) for k, v in state_Gs.items() if
        ...
        max_res = max(
            [int(re.search('(\d+)x\d+', k).group(1)) for k in param_dict.keys() if k.endswith('conv1.weight')])
        G = StyleGAN(G_mapping(), Truncation(), G_style(resolution=max_res), G_synthesis(resolution=max_res))
        G.load_state_dict(param_dict, strict=False)  # needed for the blur kernels
        print(G.state_dict().keys())
        torch.save(G.state_dict(), Path(pickle_path).with_suffix('.pt'))


# if __name__ == '__main__':
#     StyleGAN.export_from_tf("../../pretrained/stylegan1/tf_models/karras2019stylegan-celebahq-1024x1024.pkl")
    # m = StyleGAN.load_from_pth("../../pretrained/stylegan1/tf_models/wikiart_faces.pt")
